...
const databaseInstance = new Database("./src/core/database/default/default.tm");
...
databaseInstance.transaction(() => {
    ...
    const playerStatement = databaseInstance.prepare("INSERT INTO player (nation_id, first_name, last_name, birth_date, position, overall, potential, market_value) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    for (const nation of NATIONS) {
        ...
        for (const clubDataRaw of clubs) {
            ...
            SeedService.seedPlayers(playerStatement, contractStatement, faker, clubId, nationId);
        }
    }
})();
